...
module Exp (Oper(..), Exp(..), Decl(..), Program(..), opStrs, bopApp,
  manyApp, tuple, showTuple)
    where

data Oper = Plus | Times | Equal
          | And | Or | Not
          | Cons | Head | Tail deriving Eq
...
instance Show Decl where
  show (Decl name exp) = name ++ " = " ++ show exp ++ "in\n"

instance Show Program where
  show (Program decls exp) = concatMap show decls ++ show exp
--eof
